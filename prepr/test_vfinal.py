...
if run:
    ...
    if warehouse_type == "csv":
        ...
        col1, col2 = st.columns(2)
        col1.metric("Rain Model Accuracy", f"{acc:.2%}")
        col2.metric("Moisture MAE", f"{mae:.2f}")
        
        st.info("ℹ️ These predictions are based on 15 years of historical patterns (2010-2024). Dates shown are day/month only.")
    ...
    # -------------------------------------------------
        ...
        # Determine the year for API query
        current_year = datetime.now().year
        current_month = datetime.now().month
        current_day = datetime.now().day
        
        # If the requested date has already passed this year, use next year
        query_year = current_year
        if month < current_month or (month == current_month and day < current_day):
            query_year = current_year + 1
        
        try:
            start_date = datetime(query_year, month, day)
        except ValueError:
            st.error("❌ Invalid date. Please check day and month values.")
            st.stop()
        
        st.info(f"🗓️ Querying API for: {start_date.strftime('%B %d, %Y')} onwards")
